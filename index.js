...
function isNextVersionVulnerable(version) {
  ...
  if (major === 15) {
    if (isCanary) {
      // Only 15.6.0-canary.58 is safe - all other 15.x canaries are vulnerable
      if (raw === '15.6.0-canary.58') {
...
function getInstalledVersionFromPackageManager(pkgDir, packageName, packageManager) {
  ...
  try {
    ...
    switch (pm) {
      case 'pnpm': {
        // pnpm why <pkg> --json returns array of dependency paths
        ...
        if (result.status === 0 && result.stdout) {
          try {
            ...
            // pnpm why returns an array, look for the package in dependencies
        ...
        if (result.status === 0 && result.stdout) {
          try {
            const data = JSON.parse(result.stdout);
            // pnpm list --json returns an array (v10+) or object (older versions)
      ...
      case 'yarn': {
        // yarn list --pattern <pkg> --json --depth=0
        ...
        if (result.status === 0 && result.stdout) {
          // Yarn outputs newline-separated JSON objects
      ...
      case 'bun': {
        // bun pm ls returns a simple format
        ...
        if (result.status === 0 && result.stdout) {
          // Look for line containing the package
          const lines = result.stdout.split('\n');
          for (const line of lines) {
            // Format varies but typically: "package@version" or "├── package@version"
